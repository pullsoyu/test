...
<script> 
		...
		$.getJSON("<c:url value='/admin/board/openCommentNoticeBoardList.do?n=${map.BOARD_IDX}&m=${map.BOARD_KIND}' />", function(data){
			var str = "";
...
<body>
	...
	$(document).on("click", ".modal_modify_btn", function(){         
		...
		comSubmit.setUrl("<c:url value='/admin/board/updateCommentNoticeBoard.do' />");
		comSubmit.submit();
	...
	$(document).on("click", ".modify", function(){				     
		...
		function fn_openBoardUpdate(){
			...
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("<c:url value='/admin/board/openNoticeBoardUpdate.do' />");
		...
		function fn_commentDelete(COMMENT_IDX){
			...
			comSubmit.addParam("BOARD_KIND", "${map.BOARD_KIND}");
			comSubmit.addParam("BOARD_IDX", "${map.BOARD_IDX}");
			comSubmit.setUrl("<c:url value='/admin/board/commentNoticeBoardDelete.do' />");
			comSubmit.submit();
		}
		
		function fn_commentWrite(){
			var comSubmit = new ComSubmit("commentForm");
			comSubmit.setUrl("<c:url value='/admin/board/insertCommentNoticeBoard.do' />");
			comSubmit.submit();
		}
